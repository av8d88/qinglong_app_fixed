...
class Api {
  ...
  Future<HttpResponse<String>> taskLogDetail(String name, String path) async {
    if (getIt<SystemBean>(instanceName: index.toString()).isUpperVersion2_13_0()) {
      return await getIt<Http>(instanceName: index.toString()).get<String>(
        getIt<Url>(instanceName: index.toString()).taskLogDetail + Uri.encodeComponent(name) + "?path=" + Uri.encodeComponent(path),
        null,
      );
    } else {
      return await getIt<Http>(instanceName: index.toString()).get<String>(
        getIt<Url>(instanceName: index.toString()).taskLogDetail + Uri.encodeComponent(path) + "/" + Uri.encodeComponent(name),
        null,
      );
    }
  }
  ...
  Future<HttpResponse<String>> scriptDetail(String name, String? path) async {
    return await getIt<Http>(instanceName: index.toString()).get<String>(
      getIt<Url>(instanceName: index.toString()).scriptDetailForReadFile + Uri.encodeComponent(name),
      {
        "path": path,
      },
    );
